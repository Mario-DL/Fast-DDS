        ...
        const std::chrono::steady_clock::time_point& max_blocking_time_point)
{
    ...
    for (auto it = buffers.begin(); it != buffers.end(); ++it)
    {
        ...
        memcpy(pos, ((*it).data()), asio::buffer_size(*it));
        // Static cast to avoid pointer arithmetic on void*
        pos = static_cast<char*>(pos) + asio::buffer_size(*it);
    }
        ...
        while (it != *destination_locators_end)
        {
            if (IsLocatorSupported(*it))
            {
                ...
                if (shared_buffer == nullptr)
                {
                    ...
                    shared_buffer = copy_to_shared_buffer(asio_buffers, total_bytes, max_blocking_time_point);
                }
